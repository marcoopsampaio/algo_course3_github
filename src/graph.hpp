...
public:
  ...
  friend bool BFS_path_exists(Graph & gin,
    unsigned int start_node, unsigned int end_node);

  friend unsigned int BFS_count_connected_components(Graph & gin);
  ...
  friend long long int Prim_MST_weight(Graph & gin);
  friend long long int Clustering_max_spacing(Graph & gin, int k);
  friend long long int Clustering_hamming(Graph & gin, int k);
};
...
long long int Clustering_hamming(Graph & gin, int k);
