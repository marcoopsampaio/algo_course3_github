	     ...
	     bool weighted)
{
  switch(input_type)
    {
    case 0:
      this->stream_read0_graph_by_edges(in_stream, directed, weighted);
      break;
    case 1:
      ...
      if(ss >> i && ss >> j) // read pair of nodes
			{
				if(weighted) {
					ss >> weight;
					this->edges_.push_back(Edge(i,j,weight)); // Add edge
				}
				else{
...
unsigned int BFS_count_connected_components(Graph & gin){
	std::vector<std::vector<unsigned int> > all_ccs;
	BFS_all_connected_components(gin, all_ccs);
	return all_ccs.size();
}
...
bool BFS_path_exists(Graph & gin,
	unsigned int start_node, unsigned int end_node){
		//std::cout<<" Going into BFS_path_exists" <<std::endl;
		return BFS_shortest_path_length(gin, start_node,
			end_node) != std::numeric_limits<unsigned int>::max();
}
...
// Basic implementation O(m*n)
long long int Prim_MST_weight(Graph & gin){
	// Vectors to keep track of visited vertices
	std::vector<unsigned int> visited;
	std::vector<bool> visited_bool(gin.n(), false);
	visited_bool[0] = true;
	visited.push_back(0);
	// variable to prepare output
	long long int sum_weights = 0;

	while(visited.size() < gin.n()){
		unsigned int node_min;
		double min_weight = 1e50;
		//std::cout<<"New Visit"<<std::endl;
		for(auto & node : visited){
			//std::cout << "inspecting node "<<node << std::endl;
			for(auto & edge: gin.nodes_[node]){

				//std::cout << ">> inspecting edge "<< edge << std::endl;
				if(visited_bool[gin.edges_[edge].first]
					&& ! visited_bool[gin.edges_[edge].second]){
						if(gin.edges_[edge].weight < min_weight){
							min_weight = gin.edges_[edge].weight;
							node_min = gin.edges_[edge].second;
						}
				}
				else if(!visited_bool[gin.edges_[edge].first]
					&& visited_bool[gin.edges_[edge].second]){
						if(gin.edges_[edge].weight < min_weight){
							min_weight = gin.edges_[edge].weight;
							node_min = gin.edges_[edge].first;
						}
				}
			}
		}
		visited_bool[node_min] = true;
		//std::cout <<"Pushinh back node "<< node_min << std::endl;
		visited.push_back(node_min);
...
long long int Clustering_max_spacing(Graph & gin, int k){
	// Make new graph with sorted edges
	EdgesVec new_edges{gin.edges_};
	bool bfs_ran{false}, find_min_dist{false};
	long long int min_dist=std::numeric_limits<std::streamsize>::max();
	int icount{0}, inext{0};
	// Sort vector of edges
	std::sort( new_edges.begin(), new_edges.end(), compare_edges);
	// write to file and load back as a new graph
	std::string filename{"temp.txt"};
	std::ofstream out_graph(filename);
	for(auto & edge: new_edges)
		out_graph << edge.first <<"\t" << edge.second << "\t" << edge.weight << "\n";
	out_graph.close();
	// Load back as new graph
	std::ifstream graphin(filename);
	Graph g0(graphin, 0, false, true); // undirected weighted read from edges list
	graphin.close();
	// std::clog<<" ... done with loading auxiliary graph! \n "<<std::endl;
	// std::cout << "number of nodes is " << g0.n() << std::endl;
	// std::cout << "number of edges is " << g0.m() << "\n" << std::endl;

	// Now we apply clustering strategy
	Graph MST_graph;

	for(auto & edge: g0.edges_){
		//std::cout << "Here!!!"<< std::endl;
		if(!find_min_dist){
			// check if two ends of edges are already in MST_graph
			if(MST_graph.index_of_ids_.find(edge.first) !=  MST_graph.index_of_ids_.end()
				&& MST_graph.index_of_ids_.find(edge.second) !=  MST_graph.index_of_ids_.end()){
					// check if there is already a path between them
					//std::cout << "Here2!!! edge.first is "<< edge.first <<" edge.second is " << edge.second << std::endl;
					//std::cout << "Here2!!! index of edge.first is "<< MST_graph.index_of_ids_[edge.first]
					// 				  <<" index of edge.second is  " << MST_graph.index_of_ids_[edge.second] << std::endl;
					//std::cout << "length of id_nodes_ is "<< MST_graph.id_nodes_.size() << std::endl;
					if(!BFS_path_exists(MST_graph, edge.first, edge.second)){

					 		//std::cout << "Here3!!!"<< std::endl;
						 //if not, then add to MST_graph
						 MST_graph.nodes_[MST_graph.index_of_ids_[edge.first]].push_back(MST_graph.edges_.size());
						 MST_graph.nodes_[MST_graph.index_of_ids_[edge.second]].push_back(MST_graph.edges_.size());
						 MST_graph.edges_.push_back(Edge(MST_graph.index_of_ids_[edge.first],
					 		MST_graph.index_of_ids_[edge.second], edge.weight));

					 }
					 // else{
						// std::cout << "Cycle would be created!!!"<< std::endl;
					 // }
				}
			else{
				// add to MST_graph taking care of adding new node and relabeling indices
				if(MST_graph.index_of_ids_.find(edge.first) == MST_graph.index_of_ids_.end()){
				//					 		std::cout << "Here5!!!"<< std::endl;
					MST_graph.index_of_ids_[edge.first] = MST_graph.id_nodes_.size();
					MST_graph.id_nodes_.push_back(edge.first);
					MST_graph.nodes_.push_back(std::list<unsigned int>(1,MST_graph.edges_.size()));
				}
				else{
					MST_graph.nodes_[MST_graph.index_of_ids_[edge.first]].push_back(MST_graph.edges_.size());
				}
				if(MST_graph.index_of_ids_.find(edge.second) ==  MST_graph.index_of_ids_.end()){
				//					 		std::cout << "Here5!!!"<< std::endl;
					MST_graph.index_of_ids_[edge.second] = MST_graph.id_nodes_.size();
					MST_graph.id_nodes_.push_back(edge.second);
					MST_graph.nodes_.push_back(std::list<unsigned int>(1,MST_graph.edges_.size()));
				}
				else{
					MST_graph.nodes_[MST_graph.index_of_ids_[edge.second]].push_back(MST_graph.edges_.size());
			...
			if(g0.nodes_.size() - MST_graph.nodes_.size() < k && (inext == icount)){
				bfs_ran = true;
				icount = 0;
				int ncc = BFS_count_connected_components(MST_graph);
				std::cout << " upperbound nclusters is  "<< ncc + g0.nodes_.size() - MST_graph.nodes_.size()<<std::endl;
				inext = ncc + g0.nodes_.size() - MST_graph.nodes_.size() - 1 - k;
				if(inext < 0){
					inext = 0;
				}
				if(ncc + g0.nodes_.size() - MST_graph.nodes_.size() == k){
					std::cout << " number of clusters is "<< k<<std::endl;
					find_min_dist = true;
				}
			}
			else if(bfs_ran){
			...
			//std::cout << "cccount " << BFS_count_connected_components(MST_graph)<<std::endl;
		}
		else{
			if(MST_graph.index_of_ids_.find(edge.first) !=  MST_graph.index_of_ids_.end()
				&& MST_graph.index_of_ids_.find(edge.second) !=  MST_graph.index_of_ids_.end()){
	...
	//g0.print_graph(std::cout, true);
	//MST_graph.print_graph(std::cout, true);
	// Print MST graph proprties
	std::clog<<" MST graph found! \n "<<std::endl;
	std::cout << "number of nodes is " << MST_graph.n() << std::endl;
	std::cout << "number of edges is " << MST_graph.m() << "\n" << std::endl;
				std::cout << " MST ccs??  "<<  BFS_count_connected_components(MST_graph)<<std::endl;

  std::cout << "cccount_initial " << BFS_count_connected_components(g0)<<std::endl;

	std::cout << "number of nodes is " << g0.n() << std::endl;
	std::cout << "number of edges is " << g0.m() << "\n" << std::endl;


	std::cout << "min_dist is " << min_dist << "\n" << std::endl;
	...
	// get minimum distance of points in different clusters
...
long long int Clustering_hamming(Graph & g0, int k){
	// Make new graph with sorted edges
	bool bfs_ran{false}, find_min_dist{false};
	long long int min_dist=std::numeric_limits<std::streamsize>::max();
	int icount{0}, inext{0};
	...
	for(auto & edge: g0.edges_){
		...
		if(!find_min_dist){
			...
			if((g0.nodes_.size() - MST_graph.nodes_.size() < k) && (inext == icount)){
				...
				int ncc = BFS_count_connected_components(MST_graph);
				std::cout << " upperbound nclusters is  "<< ncc + g0.nodes_.size() - MST_graph.nodes_.size()<<std::endl;
				std::cout << " ncc is  "<< ncc <<std::endl;
				std::cout << " g0.nodes_.size() is  "<< g0.nodes_.size()<<std::endl;
				std::cout << " MST_graph.nodes_.size() is  "<< MST_graph.nodes_.size()<<std::endl;
